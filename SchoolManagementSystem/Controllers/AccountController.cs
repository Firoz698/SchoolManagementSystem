...
namespace SchoolManagementSystem.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> RecoverPassword(RecoverPasswordViewModel model)
        {
            ...
            // 2️⃣ Random temporary password তৈরি করুন
            var tempPassword = "Temp@12345"; // অথবা Random string generate করুন

            // 3️⃣ রিসেট টোকেন তৈরি করুন
            var token = await _userHelper.GeneratePasswordResetTokenAsync(user);

            // 4️⃣ পাসওয়ার্ড রিসেট করুন
            var resetResult = await _userHelper.ResetPasswordAsync(user, token, tempPassword);
            ...
            // 5️⃣ ই-মেইল বডি তৈরি করুন
            var body = $"<h1>Password Reset</h1>" +
                       $"Your new temporary password is: <strong>{tempPassword}</strong><br>" +
                       $"Please login and change your password immediately.";

            // 6️⃣ ই-মেইল পাঠান
            var response = _mailHelper.SendEmail(user.Email, "Your New Password", body);
            ...
            {
                ModelState.AddModelError("", "Failed to send email: " + response.Message);
            }
        ...
        //        var body = $"<h1>Password Reset</h1>" +
        //   $"Your new temporary password is: <strong>NewStaticPassword123!</strong>";

        //        Helpers.Response response = _mailHelper.SendEmail(user.Email, "Your New Password", body);
