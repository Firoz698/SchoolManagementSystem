            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (model.ImageFile != null && model.ImageFile.Length > 0)
                    {
                        ...
                        blobFile.ImageFile = await this.ConvertToByteArrayAsync(model.ImageFile);
                        blobFile.FileName = model.ImageFile.FileName;
                        blobFile.UploadedAt = DateTime.Now;
                        blobFile.GuId = Guid.NewGuid();
                        await _blobFileRepository.CreateAsync(blobFile);
                        imageId = blobFile.GuId; // A
                    }
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (model.ImageFile != null && model.ImageFile.Length > 0)
                    {
                        ...
                        imageId = blobFile.GuId; // Assuming GuId is used as the unique identifier for the image
                    }
